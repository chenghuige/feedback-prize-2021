...
def log_softmax(x, axis=-1):
  import scipy
  return scipy.special.log_softmax(x, axis)
...
#@TODO better pad
def pad(l, maxlen, pad_token=0, last_tokens=0, dtype=None, use_sep=False, sep_token=None, cls_token=None, eos_token=None):
  l = l.copy()
...
def reduce_len(l, maxlen, mark=0, last_tokens=0, use_sep=False, sep_token=None):
  if isinstance(l, np.ndarray):
    l = list(l)
...
def encode(text, tokenizer, max_len, last_tokens=0, padding=True, padding_val=0):
  input_ids = tokenizer.encode(text)
  deal_fn = pad if padding else reduce_len
  input_ids = deal_fn(input_ids, max_len, padding_val, last_tokens=last_tokens)
  return input_ids

def encode2(text1, text2, tokenizer, max_len1, max_len2, last_tokens1=0, last_tokens2=0, padding=True, padding_val=0):
  input_ids1 = tokenizer.encode(text1)
  input_ids1 = gezi.reduce_len(input_ids1, max_len1, padding_val, last_tokens=last_tokens1)
  deal_fn = pad if padding else reduce_len
  input_ids2 = tokenizer.encode(text2)
  input_ids = [*input_ids1, *input_ids2[1:]]
  input_ids = deal_fn(input_ids, max_len2, padding_val, last_tokens=last_tokens2)
  return input_ids
...
def save_huggingface(model_name, model_root, tokenizer=None, backbone=None):
...
def latest_weights(model_dir):
  files = glob.glob(f'{model_dir}/*.h5') + glob.glob(f'{model_dir}/*.tar') + glob.glob(f'{model_dir}/*.pt') + glob.glob(f'{model_dir}/*.bin')
  files = sorted(files, key=lambda x: os.path.getmtime(x))
  return files[-1]
...
def predict(model, inputs, batch_size=None, method='keras', dynamic_keys=[], mask_key=None):
  if not FLAGS.torch:
    import husky
    import melt as mt
    if dynamic_keys:
    ...
    with strategy.scope():
      if method == 'keras':
        ...
        return model.predict(inputs, batch_size=batch_size, callbacks=[husky.TQDMProgressBar('Predicting')])
      elif method == 'husky':
        ...
        if isinstance(inputs, dict):
          ...
          dataloader = inputs
        return mt.Model(model).infer(dataloader, dump_inputs=False, callbacks=[husky.TQDMProgressBar('Predicting')])
      else:
        raise ValueError(method)
  else:
    import lele
    #TODO wrap DataParallel ? now assume nn.DataParallel(model.cuda()) before input
    return lele.predict(model, inputs, batch_size=batch_size, dynamic_keys=dynamic_keys, mask_key=mask_key)

def save_model(model, model_path, fp16=False):
  if FLAGS.torch:
    import lele
...
def folds_metrics_summary(model_dir, folds='folds'):
  ...
  for fold in range(folds):
    ...
    cols = [x for x in df.columns if x not in Set(['epoch', 'insts', 'ntime', 'step'])]
    for col in cols:
      m[col] += df[col].values[-1]
      sw.log({f'Metrics/now/{col}': df[col].values[-1], 'Metrics/step': fold, 'step': fold})
      sw.log({f'Metrics/mean/{col}': m[col] / (fold + 1), 'Metrics/step': fold, 'step': fold})
...
def folds_metrics_wandb(model_dir, folds, force=False):
  ...
  lock_file = f'{root}/{folds}/{model_name}/lock'
  if not os.path.exists(lock_file) or force:
    ...
    for fold in range(folds):
      ...
      for col in cols:
        m[col] += df[col].values[-1]
        gezi.log_wandb({f'Metrics/now/{col}': df[col].values[-1], 'Metrics/step': fold, 'step': fold})
        gezi.log_wandb({f'Metrics/mean/{col}': m[col] / (fold + 1), 'Metrics/step': fold, 'step': fold})
