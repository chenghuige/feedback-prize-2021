...
def model_name_from_args(ignores=[], sep='.'):
  args = sys.argv[1:]
  ignores += ['mn', 'mns', 'mts', 'model_name', 'cf', 'clear_first', 
              'fold', 'folds', 'online', 'async_valid', 'async_eval',
              'mode', 'nvs', 'train_scratch', 'scratch', 'debug', 'test', 'restore_configs',
              'first_interval_epoch', 'fie', 'train_allnew', 'allnew', 'train_only',
              'pretrain', 'pretrained', 'gpus', 'num_gpus', 'eval_batch_size', 
              'steps', 'num_train', 'num_valid', 'folds_metrics', 'folds_metrics_force']
  for arg in args:
    ...
    for ignore in ignores:
      if arg.split('=')[0] == f'--{ignore}':
        ...
        break
    if ok:
      FLAGS.mn += sep + arg.replace('--', '').replace('=', '-').replace(' ', '-')
