...
def init(graph=None):
  ...
  if (not FLAGS.clear_first):
    ...
    if (FLAGS.train_scratch or 'SCRATCH' in os.environ) and (FLAGS.work_mode == 'train') and (FLAGS.mode == 'train' or FLAGS.mode is None) and (not FLAGS.steps < 0) and (not FLAGS.folds_metrics):
      ...
      #                   FLAGS.model_dir, 'exit(0)')
        # exit(0)
    if model_exists and FLAGS.train_allnew and (FLAGS.work_mode == 'train') and (FLAGS.mode == 'train' or FLAGS.mode is None):
      ic(FLAGS.model_dir, model_exists, FLAGS.train_all, FLAGS.train_allnew, 'ignore and exist')
      exit(0)

    if (not FLAGS.allow_loop) and model_exists and os.path.exists(f'{FLAGS.model_dir}/metrics.csv') and  (FLAGS.mode == 'train' or FLAGS.mode is None) and (not FLAGS.steps < 0) and (not FLAGS.folds_metrics):
  ...
  if not gezi.get('wandb_run'):
    try:
      ...
      if FLAGS.wandb:
        ...
        if not run:
          logging.warning('Turn off wandb due to error like fail to connect')
          FLAGS.wandb = False
