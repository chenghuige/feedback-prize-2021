...
class MMoE(nn.Module):
  def __init__(self, num_tasks, input_dim, dims=[], output_dim=None,  num_experts=None, activation='ReLU', return_att=False):
    super().__init__()
    ...
    if output_dim is None:
      output_dim = dims[-1]
    idim = output_dim
    self.gates = nn.ModuleList([nn.Sequential(nn.Linear(idim, num_experts, bias=False), nn.Softmax()) for _ in range(num_tasks)])
